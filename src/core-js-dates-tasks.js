...
function isDateInPeriod(date, period) {
  const dateMillisecs = new Date(date).getTime();
  const startMillisecs = new Date(period.start).getTime();
  const endMillisecs = new Date(period.end).getTime();
  if (dateMillisecs >= startMillisecs && dateMillisecs <= endMillisecs) {
    return true;
  }
  return false;
}
...
function getWeekNumberByDate(date) {
  ...
  const weekNumber = Math.ceil(((result - yearStart) / 86400000 + 1) / 7);
  return weekNumber;
}
...
function getNextFridayThe13th(date) {
  ...
  for (let i = 0; ; i += 1) {
    const isFriday = new Date(year, month, 13);
